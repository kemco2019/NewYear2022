...
try {
    ...
    if (isset($_POST['upload'])) {//送信ボタンが押された場合
        if (!empty($_FILES['image']['name'])) {//ファイルが選択されていれば$imageにファイル名を代入
            ...
            $img_left = imagerotate($img, 90, 0);

            $width  = 379;//Canvasの幅
            $height = 519;//Canvasの高さ
            ...
            imagecopy($create_image, $img_left, $position_x1, $position_y1, 66, 332, 379, 519);//コピー先の画像リソース, コピー元の画像リソース, コピー先の x 座標, コピー先の y 座標, コピー元の x 座標,　コピー元の y 座標, コピー元の幅, コピー元の高さ
            ...
            $sql = "INSERT INTO tora_2022 (path)  VALUES (:file)";
